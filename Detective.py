...
def Wrongcls():
    ...
    with open('./data/classification_results/classification_VOCgt_inferences.json', 'r') as f:
        cls_gt = json.load(f)
            ...
            if torch.max(IoU) > params.t_f:
                ...
                results.append(
                    {"image_name": image_name, "wrongcls_loss": loss, "bbox": box, "full_scores": cls_full_score,
                     "detectiongt_category_id": detectiongt_category})
...
def PoorLoc():
    cls_gt = []  # ./data/classification/classification_VOCgt_inferences.json
    cls_inf = []  # ./data/classification/classification_VOCinf_inferences.json
    dec = []  # ./data/detection_results/ssd_VOCval_inferences.json
    with open('./data/classification_results/classification_VOCgt_inferences.json', 'r') as f:
        cls_gt = json.load(f)
    with open('./data/classification_results/classification_VOCinf_inferences.json', 'r') as f:
        cls_inf = json.load(f)
    with open('./data/detection_results/ssd_VOCval_inferences.json', 'r') as f:
        dec = json.load(f)
    # get inference results score > m_t
            ...
            if torch.max(IoU) < params.t_b:
                ...
                results.append(
                    {"image_name": image_name, "missing_loss": loss, "bbox": box, "full_scores": cls_full_score,
                        "detectioninf_category_id": detectioninf_category})
